...
\setmargins{3cm}{0cm}{25.5cm}{16cm}{0cm}{3cm}{0cm}{0cm}
...
\begin{document}
	\begin{multicols}{2}
		...
		<cmd1> | <cmd2> & redirigie le resultat du programme \code{cmd1} vers le programme \code{cmd2}\\
		\end{tabular}
		...
		chmod & change les permissions d'un fichier\\[1mm]
		|\quad chmod +w <path> & ajoute les droits en écriture au fichier \code{path} à tout les utilisateurs\\
		...
		\end{itemize}	 	  \\
		chown & change le propriétaire d'un fichier\\[2mm]
		...
		\bigskip
		\begin{tabular}{>{\tt\footnotesize}p{4cm}p{7.5cm}}
		\multicolumn{2}{l}{{\Large\color{ecamred}\sf Gestion des utilisateurs}\medskip}\\	
		sudo & effectue une commande sous le nom du super utilisateur \textbf{root}\\[1mm]
		adduser <user> & ajoute un utilisateur portant le nom \code{user}\\
		|\quad <user> <group> & ajoute un utilisateur portant le nom \code{user} au groupe \code{group}\\
		deluser <user> & supprime l'utilisateur portant le nom \code{user}\\[1mm]
		groups <user> & liste les groupes auxquelles appartient \code{user}\\
		groupadd <group> &ajoute un groupe portant le nom \code{group}\\
		groupdel <group> & supprime le groupe portant le nom \code{group}\\
		\end{tabular}
		...
		\multicolumn{2}{l}{{\Large\color{ecamred}\sf Gestion des processus}\medskip}\\			
			...
			|\quad kill -9 <pid> & tue violemment le processus \\[2mm]
			<cmd> \& & lance la commande \code{cmd} en arrière plan\\[2mm]
			jobs & affiche la liste des jobs en cours\\[2mm]
			fg \%<jobs> & passe la commande qui a pour numéro de job \code{jobs} au premier plan
		...
		\begin{tabular}{>{\tt\footnotesize}p{4cm}p{7.5cm}}
			...
			cat >> <path> & permet d'envoyer un texte dans le fichier \code{path} à la suite du texte déjà présent.\\[2mm]
			printenv & affiche la liste des variables.\\[2mm]
			grep <regex> & affiche les lignes contenant  \code{regex}\\
			|\quad <cmd> | grep <regex> &  affiche les lignes contenant  \code{regex} issu du résultat de \code{cmd}\\
		\end{tabular}
	\end{multicols}
\end{document}